...
bool FILE_SYSTEM::process_command(string command, ostream &fout) {
	...
	int index;
	index = command.find(
			" ");                  // index = -1 if there is only one word in the command otherwise gives an index where space is located
	if (index != -1) {
	...
	if (cmd == "mkdir") {
		...
		fout << "Recursively listing all files and directories in '" << cwd->name
			 << "' directory and all subdirectories " << endl;
		print_all(root, fout);
		fout << endl;
...
void FILE_SYSTEM::make_dir(string name, ostream &fout) {
	...
	} else if (ptr == nullptr)             // if the directory is empty
	{
		...
		if (ptr->t == 'd') {
			while (ptr->sibling != nullptr && ptr->sibling->t != 'f' && ptr->sibling->name < node->name &&
				   ptr->name != node->name) {
				ptr = ptr->sibling;
			}
			if (ptr->name == node->name)                 // if a dir with a same name is encountered
			{
				...
				delete node;
				return;
			} else if (ptr->sibling == nullptr ||
					   ptr->sibling->t == 'f')            // if there is only one dir in the list
			{
